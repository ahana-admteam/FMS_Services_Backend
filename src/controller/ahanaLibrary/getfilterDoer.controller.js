...
getfilterDoer.get("/getfilterDoer", async (req, res) => {
  try {
    ...
    infoLogger.log(
      "info",
      `Username: ${userName} hit getfilterDoer with params ${JSON.stringify(
        req.query
      )}`
    );
    ...
    // Query
    const query = {
      "fmsTaskDoer.employeeId": employeeId,
    };
    ...
    errorLogger.log(
      "error",
      `Failed to fetch doer filtered fms data due to ${error.message}`
    );
