...
updateFmsTasks.post('/updateFmsTask', async (req, res) => {
  try {
    ...
    //  Find current step by matching what.what — exclude Global TAT from workflow
    const workflowSteps = fmsMasterDocument.fmsSteps.filter(
      s => s.what?.what !== "Global TAT"
    );
    ...
    //  Format requestor feedback if present on this step
    ...
    if (
      ...
      formattedFeedback = masterFeedback.map(q => {
        ...
        return { ...q, answer: answerObj ? answerObj.answer : null };
      });
    }
    ...
    if (isNegative) {
      ...
      //  Also complete the Global TAT task when flow stops early
      await completeGlobalTatTask(fmsMasterId, fmsQAId);
    ...
    //  Find next WORKFLOW step (skip Global TAT — it is handled separately)
    const nextStep = workflowSteps.find(
      step => step.step === currentStep.step + 1
    );

    // ─── LAST WORKFLOW STEP COMPLETED (step 8 in your case) ──────────────────
    if (!nextStep) {
      console.log("Last workflow step completed — closing fmsQA flow");

      //  Complete the Global TAT task and mark ONTIME or DELAY
      await completeGlobalTatTask(fmsMasterId, fmsQAId);
    ...
    if (existingTask) {
      // Task already exists — update its status based on planned time
      ...
      if (nextStep.plannedDate.type === "T-X") {
        const txSteps = fmsMasterDocument.fmsSteps.filter(
          s => s.plannedDate.type === "T-X" &&
            s.step >= nextStep.step &&
            s.what?.what !== "Global TAT"   //  Never include Global TAT in T-X creation
        );
...
// ─── Complete Global TAT Task ─────────────────────────────────────────────────
//  Called when:
//    (a) Last workflow step (step 8) is completed successfully
//    (b) Flow stops early due to a negative status (Not Available / Not Issued)
//  Marks the Global TAT task COMPLETED and sets ONTIME or DELAY
async function completeGlobalTatTask(fmsMasterId, fmsQAId) {
  try {
    const globalTatTask = await FmsTasks.findOne({
      fmsMasterId,
      fmsQAId,
      stepType: "GLOBAL_TAT"
    });
    ...
    if (!globalTatTask) {
      console.warn(`Global TAT task not found for fmsQAId ${fmsQAId} — skipping`);
      return;
    }

    const currentDate = moment().tz('Asia/Kolkata').format();

    const planned = moment(globalTatTask.fmsTaskPlannedCompletionTime);
    const actual = moment(currentDate);

    // actual - planned
    const diffMs = actual.diff(planned);

    const isOnTime = diffMs <= 0;

    const absDuration = moment.duration(Math.abs(diffMs));

    // IMPORTANT: use total hours
    const hours = String(Math.floor(absDuration.asHours())).padStart(2, '0');
    const minutes = String(absDuration.minutes()).padStart(2, '0');
    const seconds = String(absDuration.seconds()).padStart(2, '0');

    const sign = diffMs >= 0 ? "+" : "-";

    const formattedDifference = `${sign}${hours}:${minutes}:${seconds}`;

    await FmsTasks.findOneAndUpdate(
      { _id: globalTatTask._id },
      {
        $set: {
          fmsTaskStatus: "COMPLETED",
          fmsTaskCompletedTime: currentDate,
          fmsTaskCompletedStatus: isOnTime ? "ONTIME" : "DELAY",
          fmsTaskTimeDifference: formattedDifference
        }
      }
    );

    //  Update overall FMS QA status from Global TAT result
    await FmsQA.findOneAndUpdate(
      { fmsQAId },
      {
        $set: {
          fmsStatus: "COMPLETED",
          fmsCompletionstatus: isOnTime ? "ONTIME" : "DELAY",
          fmsDelayTime: formattedDifference
        }
      }
    );
    console.log(
      `Global TAT task completed — ${isOnTime ? "ONTIME" : "DELAY"}` +
      ` | Planned: ${globalTatTask.fmsTaskPlannedCompletionTime} | Actual: ${currentDate}`
    );

  } catch (error) {
    console.error("Error completing Global TAT task:", error);
    // Non-blocking — log but don't throw so the main flow response still returns
  }
}
...
async function updateTaskStatus(fmsTaskId, formStepsAnswers, fmsTaskQualityDetails, selectedStatus, formattedFeedback, completedBy) {
  try {
    ...
    //  Calculate ONTIME / DELAY + Time Difference

    const planned = moment(task.fmsTaskPlannedCompletionTime);
    const actual = moment(currentDate);
    ...
    // Convert absolute milliseconds to duration
    const absDuration = moment.duration(Math.abs(diffMs));

    // Total hours (important for >24 hrs)
    const hours = String(Math.floor(absDuration.asHours())).padStart(2, '0');
    const minutes = String(absDuration.minutes()).padStart(2, '0');
    const seconds = String(absDuration.seconds()).padStart(2, '0');

    // + for delay, - for early completion
    const sign = diffMs >= 0 ? "+" : "-";
    ...
    console.log(
      `Task ${fmsTaskId} completed — ${isOnTime ? "ONTIME" : "DELAY"} | Difference: ${formattedDifference}`
    );

    console.log(`Task ${fmsTaskId} completed — ${isOnTime ? "ONTIME" : "DELAY"}`);

    //  If this task was transferred, update the source task too
  ...
  for (let step of txSteps) {
    try {
      ...
      if (step.assignedTo === "REQUESTOR") {
        ...
        // }];
        stepDoers = [{
          empId: requestorDetails.requestorEmpId,
          empName: requestorDetails.requestorName,
          email: requestorDetails.requestorEmail
        }];
      } else {
